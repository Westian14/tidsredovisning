...
function test_HamtaEnAktivitet(): string {
    ...
    try {
        ...
        //Misslyckas hämta post id=
        $svar=hamtaEnskildAktivitet("");
        if($svar->getStatus()===400) {
            $retur .="<p class='ok'>Hämta post med tomt id misslyckades, som förväntat</p>";
        }
        else {
            $retur .="<p class='error'>Hämta post med tomt id returnerade " .$svar->getStatus()
            . " istället för förväntat 400</p>";
        }
        ...
        if($svar->getStatus()===200) {
            $retur .="<p class='ok'>Hämta en aktivitet lyckades</p>";
        }
        else {
...
function test_UppdateraAktivitet(): string {
    ...
    $svar=uppdateraAktivitet("12", "");
    if($svar->getStatus()===400) {
        $retur .="<p class='ok'>Hämta post med tom aktivitet misslyckades, som förväntat</p>";
    }
    else {
        $retur .="<p class='error'>Hämta post med tom aktivitet returnerade " .$svar->getStatus()
        . " istället för förväntat 400</p>";
    }

    //Uppdatera med samma information som misslyckas
    ...
    $svar=uppdateraAktivitet("$nyttId", "NyAktivitet" . $aktivitet);
    ...
    $nyttId++;
    $svar=uppdateraAktivitet("$nyttId", "Yappin" . $aktivitet);
    if($svar->getStatus()===200 && $svar->getContent()->result===false) {
...
function test_RaderaAktivitet(): string {
    ...
    try {
        ...
        if($svar->getStatus()===400) {
            ...
            $retur .="<p class='error'>Radera aktivitet med felaktigt id (-1) misslyckades.<br>"
        ...
        if($svar->getStatus()===400) {
            ...
            $retur .="<p class='error'>Radera aktivitet med felaktigt id (3.14) misslyckades.<br>"
            . $svar->getStatus() . "returnerades istället för förväntat 400</p>";
        }
